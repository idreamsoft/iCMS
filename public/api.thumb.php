 ...
 * 图片目录绝对路径 最后带 /
 */
define('iPHP_RES_PAHT', __DIR__ . '/../res/');
...
define('CDN_EXPIRES', 31536000); //CDN缓存时间
...
class Thumb
{
	...
	{
		if (CDN_EXPIRES) {
			header("Cache-Control:public,max-age=" . CDN_EXPIRES);
			header("Pragma: public");
			header('Last-Modified: ' . gmdate('D, d M Y H:i:s', $_SERVER['REQUEST_TIME']) . ' GMT');
			header('Expires: ' . gmdate('D, d M Y H:i:s', $_SERVER['REQUEST_TIME'] + CDN_EXPIRES) . ' GMT');
		} else {
	...
	public static function cache($path, $data = null)
	{
		...
		if ($cacheTime === false || (iPHP_RES_CACHE_TIME > 0 &&
